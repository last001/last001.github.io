<template>
  <div class="ds-wrapper application-wrapper">
    ...
    <div class="ds-desc">
      ...
      <span class="detail">用于记录系统修改、删除、标记删除等操作的详细数据，并可对删除、标记删除操作进行回退。</span>
...
const tableData = {
  queryMetaPageFun:dsmain.getDataLogMetaPage,
  queryPageInfoFun:dsmain.getDataLogPage,
  filters:[],
...
const handler = (command:any,data:any,event:any)=>{
  switch (command) {
    case "detail":
      router.push({path:"/framework/org/datalogdetail"});
      break;
    case "querySubmit":
      let adFilters = [];
...
router.addRoute("home",{
  name: 'DataLogDetail',
  path: '/framework/org/datalogdetail',
  meta: { title: "日志详情", menuAside: true,menuId:router.currentRoute.value.meta.id,iconName:router.currentRoute.value.meta.iconName },
  component: () => import('./DataLogDetail.vue')
});
//endregion
</script>
