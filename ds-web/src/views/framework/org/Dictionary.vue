        ...
        <template #operationColumn>
          <el-table-column fixed="right" label="操作" width="250">
            <template #default="scope">
              <el-button type="default" v-if="scope.row.dataType==='system'" size="small" @click.prevent="handler('viewDetail', scope, $event)">查看</el-button>
              <el-button type="default" v-if="scope.row.dataType==='user'" size="small" @click.prevent="handler('editDetail', scope, $event)">编辑</el-button>
...
const handler = (command: string, data: any, event: any) => {
  switch (command) {
    case "create":
      formData.dataType="user";
      formData.dataTypeText="用户字典";
