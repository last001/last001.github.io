    ...
    <div class="box">
      <div class="personalInfo">
        <div class="besaInfo">
          ...
          <div class="v-besainfo" v-loading="personInfoLoading">
            <div class="header">
              <span>头像：</span>
              <div class="img">
            ...
            <div class="name">
              ...
              <input
                type="text"
                placeholder="请输入"
                v-model="infoData.userPassword"
              />
              <el-button type="primary" @click="saveName()"
                >{{ saveText
                }}<i v-if="saveNameLoading" class="el-icon-loading"></i
              ></el-button>
            </div>
            <div class="email">
              <span>邮箱：</span>
...
export default {
  ...
  methods: {
      ...
      this.$axios(uploadFiles.uploadFile(formData))
        .then((result) => {
          ...
          if (result.data.Code == "200") {
            this.InfoData.img = this.infoData.img = result.data.imgsURL;
            let list = {
    ...
    // 保存个人信息
    savePersonInfo(string) {
      let data = {
        img: this.infoData.img,
        userId: this.infoData.id,
        password: this.infoData.userPassword,
        phone: this.infoData.phoneNumber,
        mail: this.infoData.mail,
      };
      this.$axios({
