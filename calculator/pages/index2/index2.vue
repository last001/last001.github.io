		...
		<view class="tariff">
			<view class="type_declaration">
				...
				<view class="declaration">
					...
					<view class="int_unit">
						<input type="digit" v-model="declaration" />
				...
				<view class="list" v-for="(item, index) in tariffList" :key="index">
					...
					<view class="salePrice_number">
						<view class="salePrice">
							...
							<input type="digit" v-model="item.salePrice" />
							<view class="unit">元</view>
						</view>
						<view class="number">
							<text class="text">件数：</text>
							<input type="digit" v-model="item.count" />
...
export default {
	...
	methods: {
		...
		onInput(e) {
			...
			this.allBubbleWeight = ((Number(this.length) * Number(this.width) * Number(this.height)) / 6000).toFixed(2);
		},
		//添加箱数
		addBtn() {
			this.boxCount++;
		...
		getResult() {
			...
			if (Number(this.weight) >= Number(this.allBubbleWeight)) {
				this.result = Number(this.weight) * Number(this.price) * Number(this.boxCount);
			} else {
				this.result = Number(this.allBubbleWeight) * Number(this.price) * Number(this.boxCount);
			}
